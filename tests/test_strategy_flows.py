...
def test_multiple_gates_all_appear_in_filter_block():
    ...
    strat._model = _AlwaysMinModel()
    strat._scaler = _PassthroughScaler()
    strat._candles = [_candle(100.0, volume=1000) for _ in range(25)]
...
def test_features_insufficient_candles_no_signal():
    ...
    strat._candles = [_candle(100.0) for _ in range(10)]
...
def test_entry_blocked_outside_trading_window():
    ...
    strat = _ready_strategy()  # default window 09:30–15:30
...
def test_hold_bars_exit_fires_outside_trading_window():
    """Exits (hold-bars timeout) must fire regardless of trading window — the window
